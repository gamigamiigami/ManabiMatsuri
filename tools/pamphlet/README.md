...
A2パネル下辺の紋章2つは「パンフレットをスマホ画面に重ねる謎」に使うため、
**印刷の縮尺が設計どおりでないと謎が解けなくなる**。

- 拡大縮小は必ず **100%（実際のサイズ）**。「用紙に合わせる」は使わない。
- 用紙は **A4・横**。
- 印刷したら、**下辺の「⤵ ここでうしろに折る」の点線で、下の帯（10.4mm）を
  うしろに折る**。折ったあとの紙のはしが、紋章をちょうど半分にする線になる。
  折らないと謎は解けないので、配る前に必ず折っておくこと。
  ハサミは不要。折るだけなので裏面も失われず、元に戻せる。
- 紙の下辺そのものを使っていないのは、フチなし印刷ができないプリンタだと
  下辺ぎりぎりが印刷されず、紋章の下半分が出ないため。
- 設計値：魔法陣のインク直径 約17.8mm、鍵のインク 約21.1 x 13.6mm、
...
- **モチーフPNGは canvas いっぱいに描かれていない。** 鍵は 2400px 中
  x 0.19〜0.87 の範囲にしか絵がない。canvas の中心を線に合わせると
  鍵だけ線からずれる（実際にこれで一度失敗している）。
  スクリプトの `INK` テーブルがインクの範囲を持っている。
- **紙に刷る紋章は rotate(-44.27deg) しておく。** 参加者は紙を折り線の
  角度だけ回して置くので、刷った絵も一緒に回ってしまう。逆に回しておかないと
  向きが合わない。魔法陣は同心円なので気づきにくいが、鍵は明らかにずれる。
