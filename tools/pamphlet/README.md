...
## 概要
応時中学校「親子学び祭」のイベント用A4三つ折りパンフレット。A面（おもて）3パネル＋B面（うら）2パネルの構成。Claude Codeで編集しやすいようにレイヤー・パーツごとに分離した構造で提供。
...
```css
--fold-1: 633.5px;   /* 1本目＝謎の折り目。三等分なら640px */
--fold-2: 1318.9px;  /* 2本目。三等分なら1280px。ここは見た目だけ */
```

いまの値は、実際に刷ってみて真ん中の面を**左に1mm・右に2mm**広げたもの
（640 → 633.5 ／ 1306 → 1318.9）。面の幅は 633.5 / 685.4 / 601.1px になります。
...
- `--fold-1` は「パンフレットを手紙に重ねる謎」の折り目そのもの。
  ここを動かしたら `solve_overlay.py` の `EDGE_X_PX`（と `PANEL_W_PX`）を
  同じ値にして流し直し、出力を貼り替えること。矢印のSVGは枠ごと同じだけ
  左右に寄せる（中の座標 x=120 が折り目、という関係を保つ）。
  やらないと謎が成立しない
