// ========================================
// 謎データ（このファイルだけ差し替えれば謎の中身を変更できる）
// ========================================

// ---------------------------------------------------------------
// 🖼 謎画像の命名ルール（A〜E ＝ ①〜⑤）
//
//   images/KeyA.png 〜 KeyE.png   … 鍵謎の画像（①〜⑤のページに表示）
//   images/PlaceA.png 〜 PlaceE.png … 在り処謎の画像
//       （①〜⑤の鍵謎を解いたあとに表示。次の場所への誘導。
//         例：PlaceA＝①をクリアした人に見せる「②への在り処謎」）
//
// 画像の中身を差し替えたいときは、同じファイル名で images/ に
// 上書きアップロードするだけでOK（このファイルの編集は不要）。
// 画像がまだ無い場合は自動で非表示になる（エラーにはならない）。
// ---------------------------------------------------------------
const KEY_IMAGES = {
  1: "images/KeyA.png",
  2: "images/KeyB.png",
  3: "images/KeyC.png",
  4: "images/KeyD.png",
  5: "images/KeyE.png",
};

const PLACE_IMAGES = {
  1: "images/PlaceA.png",
  2: "images/PlaceB.png",
  3: "images/PlaceC.png",
  4: "images/PlaceD.png",
  5: "images/PlaceE.png",
};

const PUZZLES = {
  1: {
    place: "3-5 時間割表",
    title: "第一の封印",
    question: "封印の紋様が浮かびあがった……\n丸く並んだ文字を正しい順に読め。",
    image: KEY_IMAGES[1],
    arikaImage: PLACE_IMAGES[2],
    choices: ["1", "2", "3", "4"],
    answerIndex: 2,
    smallHint: "順番に読もう",
    arikaHint: "どこかで見たような…",
    status: "完成",
  },
  2: {
    place: "3-6 時計",
    title: "第二の封印",
    question: "封印の紋様が浮かびあがった……\n法則を見つけ、答えに合うイラストを選べ。",
    image: KEY_IMAGES[2],
    arikaImage: PLACE_IMAGES[3],
    choices: ["1", "2", "3", "4"],
    answerIndex: 3,
    smallHint: "左右で○の数が違う…",
    arikaHint: "どこかで見たような…",
    status: "完成",
  },
  3: {
    place: "わたり廊下",
    title: "第三の封印",
    question: "封印の紋様が浮かびあがった……\n謎をとき、正しい答えを選べ。",
    image: KEY_IMAGES[3],
    arikaImage: PLACE_IMAGES[4],
    choices: ["1", "2", "3", "4"],
    answerIndex: 0,
    smallHint: "どこかで見たような…",
    arikaHint: "どこかで見たような…",
    status: "画像未着・完成",
  },
  4: {
    place: "3-5 うしろの黒板",
    title: "第四の封印",
    question: "封印の紋様が浮かびあがった……\n謎をとき、正しい答えを選べ。",
    image: KEY_IMAGES[4],
    arikaImage: PLACE_IMAGES[5],
    choices: ["1", "2", "3", "4"],
    answerIndex: 0,
    smallHint: "指示をよく読もう",
    arikaHint: "どこかで見たような…",
    status: "画像未着・完成",
  },
  5: {
    place: "音楽室（しょうぞう画）",
    title: "第五の封印",
    question: "封印の紋様が浮かびあがった……\n色の法則を見抜き、答えに合うイラストを選べ。",
    image: KEY_IMAGES[5],
    arikaImage: PLACE_IMAGES[1],
    // PlaceA.png は画像の下半分に五十音表のヒントが埋め込まれている。
    // 通常時は上半分（謎部分）だけを見せ、ヒントボタンを押したら
    // 下半分（表）を切り出して見せる（point.htmlのCSSクロップで実現）。
    arikaHintIsImage: true,
    choices: ["1", "2", "3", "4"],
    answerIndex: 2,
    smallHint: "どんな言葉が入るのだろう…",
    arikaHint: "何か法則はないか…？",
    status: "完成",
  },
};

// 巡回順
const POINT_ORDER = [1, 2, 3, 4, 5];

// ---------------------------------------------------------------
// 🃏 ダミー謎（クリアには不要な、探索好きのための追加コンテンツ）
//
// ・QRコードは dummy.html?d=1 のような飛び先で作る（QR印刷ツールで自動生成）
// ・読み込むと「これはダミーだ。それでも解く？」と確認してから挑戦できる
// ・解いた数はホーム画面に「x / 総数」で表示される
// ・総数はこの表に書いた数がそのまま反映される（増減はここに足す/消すだけ）
// ・画像は images/DummyA.png, DummyB.png, DummyC.png …（A=1, B=2, …。無ければ自動非表示）
// ---------------------------------------------------------------
const DUMMY_PUZZLES = {
  1: {
    title: "まやかしの封印・壱",
    question: "【仮】ダミー謎1の問題文",
    image: "images/DummyA.png",
    choices: ["【仮】選択肢A", "【仮】選択肢B", "【仮】選択肢C", "【仮】選択肢D"],
    answerIndex: 0,
    smallHint: "【仮】小ヒント（未設定）",
  },
  2: {
    title: "まやかしの封印・弐",
    question: "【仮】ダミー謎2の問題文",
    image: "images/DummyB.png",
    choices: ["【仮】選択肢A", "【仮】選択肢B", "【仮】選択肢C", "【仮】選択肢D"],
    answerIndex: 0,
    smallHint: "【仮】小ヒント（未設定）",
  },
  3: {
    title: "まやかしの封印・参",
    question: "【仮】ダミー謎3の問題文",
    image: "images/DummyC.png",
    choices: ["【仮】選択肢A", "【仮】選択肢B", "【仮】選択肢C", "【仮】選択肢D"],
    answerIndex: 0,
    smallHint: "【仮】小ヒント（未設定）",
  },
};

// ダミー謎の総数（自動計算。触らなくてよい）
const DUMMY_TOTAL = Object.keys(DUMMY_PUZZLES).length;

// プロローグ（Xからの手紙）。登録画面とホームの「手紙を読みかえす」でいつでも表示される。
const PROLOGUE =
  "\n" +
  "やあ。\n" +
  "ぼくの名前はX。もうすぐ中学生になる。\n" +
  "\n" +
  // ⚠ この4〜6行目を変更したことで、パンフレット側の矢印の位置が
  //    まだ合っていない（tools/pamphlet/pamphlet.html は旧文面の座標のまま）。
  //    「や」を「なやんで」から取る配置には追加のひらがな化が必要で、
  //    案A（上から おやこ と読める／変更3箇所）か
  //    案B（矢印に①②③／変更2箇所）かを決めてから再計算する。
  "今日ね、この学校から声が聞こえたから\n" +
  "みんなのことをずっと見てたんだ。\n" +
  // 「悩んで」はパンフレット合わせの謎で「や」を指すため、あえてひらがなにしている。
  "授業でなやんで、考えて、笑って。\n" +
  "なんであんなに楽しそうなの？\n" +
  "\n" +
  // 「思ってた」はパンフレット合わせの謎で「お」を指すため、あえてひらがなにしている。
  // ここを漢字に戻すと謎の答え（おやこ）が成立しなくなるので注意。
  "ぼくは正直、ずっとおもってた。\n" +
  "「勉強なんてつまらない。\n" +
  "　学校なんて意味がない」って。\n" +
  "\n" +
  "だから試してやろうと思って、魔法をかけた。\n" +
  "学校から出られないようにしてやった。\n" +
  "\n" +
  "解けるものなら、解いてみろ。\n" +
  "\n" +
  "── 学校ぎらいのXより";

// 手紙の続き（スタートQRを読み込んだときに表示されるXのセリフ）。
// 小謎プリントを解いた参加者が、運営から受け取ったQRを読むと表示される。
const START_MESSAGE =
  "\n" +
  "……へえ。やるじゃん。\n" +
  "まさか、最初の謎を解いてくるとはね。\n" +
  "\n" +
  "でも、ここからが本番だ。\n" +
  "ぼくが学校にかけた封印は、ぜんぶで5つ。\n" +
  "封印を解く「鍵」は、\n" +
  "校内のどこかに隠させてもらった。\n" +
  "\n" +
  "5つの鍵がそろったとき、\n" +
  "最後の扉が姿を現す。\n" +
  "\n" +
  "まずは、最初の鍵だ。\n" +
  "下の《在り処謎》が、隠し場所を示している。\n" +
  "\n" +
  "……迷わずたどり着けるかな？\n" +
  "\n" +
  "── X";

// 3通目の手紙（final.html：扉の謎に挑む前に表示）。
// まだ本心は明かさず、「扉を開けてみせろ」という機能的な挑発にとどめる。
// ※ 魔法使いの扉なので、暗証番号／パスワードのような言い方はしない。
//   「配布プリント」「計算」など具体的な手段にも触れず、
//   あくまで「5つの鍵の力を使って開けろ」という魔法的な言い回しにする。
const ARRIVAL_MESSAGE =
  "\n" +
  "扉の前まで来たか。\n" +
  "\n" +
  "これは、ただの錠じゃない。魔法の扉だ。\n" +
  "鍵穴なんて、どこにもありゃしない。\n" +
  "\n" +
  "開けるには、鍵の力を借りるしかない。\n" +
  "5つの鍵に宿る力を、ひとつに束ねてみせろ。\n" +
  "\n" +
  "その力が、3つの数字となって扉に宿ったとき、\n" +
  "扉はひとりでに開くだろう。\n" +
  "\n" +
  "……さて、開けられるかな？\n" +
  "\n" +
  "── X";

// 4通目の手紙（扉の謎を解いたあと・final.html成功画面で表示）。
// Xが封印をかけた本当の理由＝親に言われた「学校の秘密」を探して見つけられなかったこと。
// 最後の謎の答え（親子と学校の絆）をXが用意できない理由がここで明かされる。
const FINALE_MESSAGE =
  "\n" +
  "最後の扉まで来たか。……やるじゃん。\n" +
  "\n" +
  "ここまで来たなら、教えてあげる。\n" +
  "ぼくがこの学校を封印した、本当の理由。\n" +
  "\n" +
  "ぼくの親はさ、いつも言うんだ。\n" +
  "「学校には秘密がある。\n" +
  "　あなたをもっと大きく成長させてくれる、\n" +
  "　何かが。だから、行きなさい」って。\n" +
  "\n" +
  "……探したよ。何度も。\n" +
  "教室も、廊下も、時間割のすみっこまで。\n" +
  "でも、どこにもなかった。\n" +
  "それに授業は難しいし、退屈だし。\n" +
  "秘密どころか、いいことなんてひとつもない。\n" +
  "\n" +
  "だから決めたんだ。\n" +
  "学校なんて行く必要ないって。\n" +
  "\n" +
  "それなのに、今日の君たちは、なんなんだよ。\n" +
  "なんでそんなに、楽しそうなんだよ。\n" +
  "\n" +
  "——だったら、わかるはずだろ？\n" +
  "そんなに学校が楽しいなら。\n" +
  "「学校の秘密」とやらを、見つけてもってこい！\n" +
  "\n" +
  "見つけられたら、封印を解いてやる。\n" +
  "これが最後の勝負だ！！\n" +
  "\n" +
  "── X";

// ========================================
// 扉の謎（final.html）
// 5つの鍵の数字を、配布プリントの演算記号で計算すると出てくる3桁の数字。
// 正解すると「Xの部屋の扉」が開く……が、これは中謎のラストであり、
// 本当の最終謎（大謎・「学校の秘密」の答え）はこの先にまだ続く。
// ========================================
const FINAL_ANSWER = "135"; // (34+11)*3=45*3=135
const FINAL_HINT = "それぞれ何が入るんだ…？";
const FINAL_SUCCESS_MESSAGE =
  "扉の向こうから、ひとつの声が聞こえた――\n" +
  "「……よく、ここまで来たね」\n" +
  "\n" +
  "中謎の封印がすべて解かれた。\n" +
  "だが——本当の封印は、まだこの先にある。";

// エンディング（Xからの最後の手紙）。
// ※ フィナーレ（③大謎クリア後）用。ネタバレ防止のため、現時点ではサイトのどこにも表示していない。
//    大謎の画面を作るときにここから読み込む。
const EPILOGUE =
  "\n" +
  "……完敗だ。\n" +
  "\n" +
  "「見方・考え方」か。\n" +
  "\n" +
  "ぼく、ずっと思ってたんだ。\n" +
  "勉強って「答えを覚えること」でしょ、って。\n" +
  "そんなの面白くもなんともないもん。\n" +
  "\n" +
  "でも今日みんなを見てて、\n" +
  "なんか違うなって感じた。\n" +
  "みんなは答えを探してたんじゃなくて、\n" +
  "一緒に考えることを楽しんでた。\n" +
  "大人も子どもも、同じ顔して悩んでた。\n" +
  "\n" +
  "それって……ぼくのきらいな勉強じゃなくて、\n" +
  "冒険みたいだ。\n" +
  "\n" +
  "なんだ。学ぶのって楽しいんだね。\n" +
  "中学生になるのが楽しみになってきたよ。\n" +
  "\n" +
  "一緒に遊んでくれてありがとう。\n" +
  "\n" +
  "── 学校楽しみなXより";

// 全クリア時に表示するメッセージ
const CLEAR_MESSAGE =
  "5つの鍵がそろった！\n鍵の力を束ねて、最後の扉に挑め！";

// ========================================
// 小謎（謎解きシートの謎。QRを読む前に解く導入パズル）
// ========================================
// 答えの表記ゆれを許容するため、各問の正解パターンを配列で定義。
// 判定時はトリムして小文字化＋全角→半角変換してから比較する。
// QRを読んだ直後に表示するXのメッセージ（小謎に挑戦する前）
const KOZO_CHALLENGE_MESSAGE =
  "\n" +
  "やあ、来たね。\n" +
  "\n" +
  "手紙は読んだかい？\n" +
  "まずは小手調べだ。\n" +
  "配られたシートの謎を解いてみろ。\n" +
  "\n" +
  "── X";

// 小謎クリア後に表示するXのメッセージ（5つの鍵の説明＋在り処謎へ）
const KOZO_CLEARED_MESSAGE =
  "\n" +
  "……へえ、やるじゃん。\n" +
  "\n" +
  "だが、ここからが本番だ。\n" +
  "封印を解くには、まず僕のところに来てみろ。\n" +
  "この部屋に入るためには、\n" +
  "この学校に隠した「5つの鍵」を見つけること。\n" +
  "\n" +
  "下の《在り処謎》が、\n" +
  "最初の鍵の隠し場所を示している。\n" +
  "……たどり着けるかな？\n" +
  "\n" +
  "── X";

const SMALL_PUZZLE_ANSWERS = [
  ["+", "＋", "ぷらす", "プラス", "plus", "たす", "タス"],
  ["×", "✕", "*", "＊", "かける", "カケル", "かけ算", "かけざん"]
];

// 小謎を正解した後のスタート地点（2グループに分けて別の場所から開始）
// QRを2種類用意し、配布時点でグループを決定する。
//   グループA用QR: index.html?skip=1&route=A → 鍵謎1の在り処謎を表示
//   グループB用QR: index.html?skip=1&route=B → 鍵謎3の在り処謎を表示
const START_ROUTES = { A: 1, B: 3 };
