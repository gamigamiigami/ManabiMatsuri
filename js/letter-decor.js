...
(function (global) {
  ...
  var PRESETS = {
    ...
    //
    // パンフレットの切り取り線にあたる直線（45.58°）の上に、
    // 魔法陣と鍵の2つを離して置く。参加者はこの2つに、パンフレットの
    // 下辺で半分に切れた同じ図形を重ねる。2点合わせれば
    ...
    //   鍵は canvas 2400px 中 x 0.19〜0.87 の範囲にしか絵がないので、
    //   ここを無視すると鍵だけ線からずれる。
    // ※ この配置は紙の縦横比が 3:4（縦向き）であることを前提にしている。
    // ------------------------------------------------------------------
    puzzleLine: [
      "magicCircle|28.57% auto|-0.864% 3.759%",
      "keyWatermark|46.60% auto|105.403% 68.414%",
    ],
  };
