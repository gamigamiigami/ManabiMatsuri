...
(function (global) {
  ...
  var PRESETS = {
    ...
    //
    // パンフレットのフチにあたる直線
    //     y = 1.628x - 0.139   （紙の幅を1、左上を原点とした座標）
    // の上に図形3つを並べる。参加者はこの3つにパンフレットの
    // 見切れた図形を重ねることで、位置・角度・拡大率を確定させる。
    // その状態でパンフレットの矢印①②③が指す文字が「お」「や」「こ」。
    //
    // 位置は背景画像の配置％に換算済み。図形の大きさや線の式を変えると
    // 答えがずれるので、変更するときは矢印の位置も再計算すること。
    // ※ この配置は紙の縦横比が 3:4（縦向き）であることを前提にしている。
    // ------------------------------------------------------------------
    puzzleLine: [
      "magicCircle|18% auto|6.7% 0.6%",
      "keyWatermark|18% auto|57.7% 59.6%",
      "grimoire|18% auto|87.4% 90.4%",
    ],
  };
