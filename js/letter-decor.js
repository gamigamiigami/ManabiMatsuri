...
(function (global) {
  ...
  var MOTIFS = {
    ...
    // ---- 25b. 重ね合わせ用の本 ----
    // ※ 一通目の手紙とパンフレットを重ねる謎「専用」のモチーフ。
    //    見た目は魔導書（grimoire）と同じ開いた本だが、こちらは
    //    viewBox を正方形にしてある。パンフレット側は同じ絵を
    //    2400x2400 の PNG（images/motifs/tome-bold.png）で刷るので、
    //    画面と紙で図形の縦横比・インク位置が完全に一致する必要がある。
    //    grimoire は viewBox が 172x72 の横長で、CSS の
    //    background-size:N% auto と正方形PNGとで基準がずれてしまう。
    //    絵を変えたら tools/motifs/render_tome.mjs で PNG を焼き直し、
    //    solve_overlay.py の INK["tome"] を測り直すこと。
    tome: {
      name: "本（重ね合わせ用）",
  ...
  var PRESETS = {
    ...
    //   面のある「本」に替えた。本は grimoire ではなく専用の tome を使う
    //   （viewBox が正方形で、印刷用PNGと幾何が一致するため）。
    ...
    puzzleLine: [
      "magicCircle|21.16% auto|0.219% 9.443%",
      "tome|34.42% auto|77.845% 49.378%",
    ],
    // ------------------------------------------------------------------
    // 一通目以外の手紙用
    //
    ...
    //   だから以下のプリセットでは magicCircle と tome を使わない。
    //   本に見える grimoire（魔導書）も、tome と紛らわしいので使わない。
    // ------------------------------------------------------------------
