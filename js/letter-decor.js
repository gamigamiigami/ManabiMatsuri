...
(function (global) {
  ...
  var MOTIFS = {
    ...
    grimoire: {
      name: "魔導書",
      note: "開いた本。「Xの研究ノート」感が出る。",
      size: "72% auto",
      position: "center",
      repeat: "no-repeat",
      // 本の上に浮かぶ小さな魔法陣は、魔法陣モチーフと紛らわしいので入れない。
      // viewBox は本だけがぴったり収まる範囲に切ってある。
      svg: svg("14 100 172 72",
    ...
    // ---- 25c. 重ね合わせ用の魔法陣・その2 ----
    // ※ 一通目の手紙とパンフレットを重ねる謎「専用」。
    //    折り目をはさんで並ぶもう一方（magicCircle）と対になる。
    //    - magicCircle  … 六芒星（三角形が2枚）＋二重の外周
    //    - magicCircle2 … 五芒星＋破線の輪＋四隅の小円
    //    どちらも魔法陣だが骨格がまったく違うので、半分に切れていても
    //    「どっちの半分か」がひと目で分かる。参加者は2つとも
    //    「1つの完全な図形」に見えるよう合わせることになる。
    //
    //    viewBox は magicCircle と同じ 200x200 の正方形。パンフレット側は
    //    同じ絵を 2400x2400 の PNG で刷るので、画面と紙で図形の縦横比・
    //    インク位置が完全に一致していなければならない。
    //    絵を変えたら tools/motifs/render_motif.mjs で PNG を焼き直し、
    //    solve_overlay.py の INK / INK_PNG を測り直すこと。
    magicCircle2: {
      name: "魔法陣・その2（重ね合わせ用）",
      note: "五芒星の陣。一通目の手紙とパンフレットを重ねる謎に使う。",
      size: "44% auto",
      position: "center",
      repeat: "no-repeat",
      svg: svg("0 0 200 200",
        '<g fill="none" stroke="' + INK_SOFT + '" stroke-width="2.2">' +
        '<circle cx="100" cy="100" r="94"/>' +
        '<circle cx="100" cy="100" r="80"/>' +
        '<polygon points="' + starPath(100, 100, 80, 5, 2) + '"/>' +
        '<circle cx="100" cy="100" r="26"/>' +
        "</g>" +
        // 破線の輪。六芒星のほうには無いので、半分だけでも見分けがつく
        '<g fill="none" stroke="' + INK_FAINT + '" stroke-width="1.4" stroke-dasharray="5 6">' +
        '<circle cx="100" cy="100" r="60"/>' +
        "</g>" +
        // 五芒星の頂点に小円
        '<g fill="none" stroke="' + INK_SOFT + '" stroke-width="1.4">' +
        ring(100, 100, 80, 5, function (x, y) {
    ...
    // ---- 25b. 正方形の本 ----
    // ※ もとは一通目の手紙とパンフレットを重ねる謎に使っていたが、
    //    いまは2種類の魔法陣（magicCircle / magicCircle2）に替えたので、
    //    謎そのものには使っていない。ふつうの透かしとして残してある。
    //    見た目は魔導書（grimoire）と同じ開いた本だが、こちらは
    //    見た目は魔導書（grimoire）と同じ開いた本だが、こちらは
    //    viewBox を正方形にしてある（grimoire は 172x72 の横長で、
    //    CSS の background-size:N% auto と正方形PNGとで基準がずれる）。
    //    PNG が要るときは node tools/motifs/render_motif.mjs tome で焼く。
    tome: {
      name: "本（重ね合わせ用）",
      note: "開いた本。一通目の手紙とパンフレットを重ねる謎に使う。",
      size: "44% auto",
      position: "center",
      repeat: "no-repeat",
      svg: svg("0 0 200 200",
        '<g fill="none" stroke="' + INK_SOFT + '" stroke-width="2.2" stroke-linejoin="round">' +
        // 左ページ
        '<path d="M100,72 C74,56 42,54 14,60 L14,132 C42,126 74,128 100,144 Z"/>' +
        // 右ページ
        '<path d="M100,72 C126,56 158,54 186,60 L186,132 C158,126 126,128 100,144 Z"/>' +
        // 背
        '<line x1="100" y1="72" x2="100" y2="144"/>' +
        "</g>" +
        // ページの罫（左右で行の傾きを変え、上下・左右どちらに切っても
        // どの半分か分かるようにしてある）
        '<g fill="none" stroke="' + INK_FAINT + '" stroke-width="1.4">' +
        '<line x1="28" y1="76" x2="86" y2="88"/>' +
        '<line x1="28" y1="92" x2="86" y2="104"/>' +
        '<line x1="28" y1="108" x2="70" y2="118"/>' +
        '<line x1="114" y1="88" x2="172" y2="76"/>' +
        '<line x1="114" y1="104" x2="172" y2="92"/>' +
        '<line x1="114" y1="118" x2="156" y2="108"/>' +
        "</g>"),
    },
  ...
  var PRESETS = {
    ...
    // パンフレット合わせ用（謎のギミック）
    //
    // 4つの図形を手紙の中にばらばらに、それぞれ「全体」が見える形で置く。
    // パンフレットの端には同じ図形の「半分」を印刷しておき、
    // 画面の図形にぴったり重なるようにパンフレットを置くと、
    // パンフレットの位置と角度が一意に決まる（斜め置きもできる）。
    // その状態でパンフレットの矢印が指す先を読むと答えがわかる。
    //
    // ・図形は4つとも離して置く（1つに合わせている間、他が隠れないように）
    // ・大きさは画面幅に対する%。参加者が画面をズームして
    ...
    // ------------------------------------------------------------------
    // 謎用（letter.html の一通目だけに使う）
    //
    // パンフレットの折り目（三つ折りの折り目）にあたる直線（44.27°）の
    // 上に、魔法陣と本の2つを離して置く。参加者はこの2つに、
    // パンフレットの折り目で半分になった同じ図形を重ねる。2点合わせれば
    // パンフレットの位置・角度・拡大率が一意に決まる。
    // 紙は文字の「左下」を覆い、文字は右上側に見える。
    //
    // その状態で矢印がさす文字は、切り取り線に沿って
    //   上から: こ → や → お
    //   下から: お → や → こ  ＝ こたえ「おやこ」
    // 3文字が一直線に並ぶので、矢印に番号を振らなくても順に読める。
    //
    // ▼ 数値の出どころ（手で書き換えないこと）
    //   tools/pamphlet/solve_overlay.py が、手紙を実際に描画して測った
    //   文字の座標とモチーフPNGのインク範囲から計算している。
    //   配置を変えたいときはあのスクリプトのパラメータを直して
    //   実行し、出てきた行をこことパンフレット側の両方に貼り直す。
    //
    // ※ background-position は「canvas の中心」ではなく
    //   「絵が実際に描かれている範囲の中心」が線に乗るように逆算してある。
    //   本は canvas 2400px 中 y 0.28〜0.72 の範囲にしか絵がないので、
    //   ここを無視すると本だけ線からずれる。
    // ※ 右下は以前は鍵だったが、鍵は細い一本線なので重ねる目印にしづらく、
    //   面のある「本」に替え、さらにいまは2種類の魔法陣に替えた。
    //   六芒星と五芒星なので、半分だけでもどちらの半分か分かる。
    // ※ この配置は紙の縦横比が 3:4（縦向き）であることを前提にしている。
    // ------------------------------------------------------------------
    // ▼ この2行は tools/pamphlet/solve_overlay.py が出した値をそのまま貼る。
    //   手で書き換えると矢印が別の文字を指すようになる。
    //   手紙の文面（PROLOGUE）を変えたら必ず出し直すこと。
    puzzleLine: [
      "magicCircle|21.16% auto|0.214% 9.439%",
      "magicCircle2|21.02% auto|73.121% 49.523%",
    ],
    // ------------------------------------------------------------------
    // 一通目以外の手紙用
    //
    // ▼ 魔法陣と本の2つがそろって描かれるのは一通目（letter.html）だけにする。
    //   この2つはパンフレットを重ねる目印なので、他の手紙にも出てくると
    //   参加者が「ここでも重ねるのか？」と迷ってしまう。
    //   だから以下のプリセットでは magicCircle と magicCircle2 を使わない。
    // ------------------------------------------------------------------
