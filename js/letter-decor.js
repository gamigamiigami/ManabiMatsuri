...
(function (global) {
  ...
  var PRESETS = {
    ...
    //
    // パンフレットのフチにあたる直線
    //     y = 0.944x + 0.064   （紙の幅を1、左上を原点とした座標／43.3°）
    // の上に図形3つを並べる。参加者はこの3つにパンフレットの
    // 見切れた図形を重ねることで、位置・角度・拡大率を確定させる。
    //
    // その状態で矢印がさす文字は、フチに沿って
    //   上から: こ → や → お
    //   下から: お → や → こ  ＝ こたえ「おやこ」
    // 3文字が一直線に並ぶので、矢印に番号を振らなくても順に読める。
    //
    // 位置は背景画像の配置％に換算済み。図形の大きさや線の式を変えると
    // 答えがずれるので、変更するときはパンフレット側も再計算すること。
    // ※ この配置は紙の縦横比が 3:4（縦向き）であることを前提にしている。
    // ------------------------------------------------------------------
    puzzleLine: [
      "magicCircle|18% auto|3.7% 7.6%",
      "keyWatermark|18% auto|47.6% 37.0%",
      "grimoire|18% auto|96.3% 68.1%",
    ],
  };
