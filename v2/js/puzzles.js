...
export const CROSSWORD = {
  // ■ 二組で「全く同じ一枚」を使ふ。ここを組ごとに分けてはならない。
  //
  //   盤面・黒枡の位置・あらかじめ入つてゐる字・手掛かりの一文、
  //   すべて両組で同一である。違ふのは室に貼られた掲示だけ
  //   （担当の者の名、室の名など、栞に載る公開情報）。
  //   同じ問ひを、自分の室の掲示で解けば自分の組の答へに、
  //   相手の室の掲示で解けば相手の組の答へに辿り着く。
  //
  //   組ごとに別の盤面を渡すと、「解き直し」が
  //   ただの二問目に見えてしまふ。
  //   同じ一枚が二通りに読めると分かる瞬間こそが、
  //   「二つの記録は、元は一つ」といふ趣旨の要である。
  //
  //   あらかじめ入つてゐる字（prefilled）には、
  //   どちらかの組の答へを匂はせる字を置かないこと。
  //   置くと、解く前に片方の答へが割れる。
  common: {
    cols: 5,
    rows: 5,
    ...
    // 手掛かりは「どの掲示を見るか」だけを言ひ、
    // どちらの室かは言はない。だから両組にそのまま渡せる。
    clue: '室の掲示から、担当の者の名と室の名を拾ひ、盤面の順に写せ。',
    answerLength: 4,
  },
};
