...
- 代はりに、決められた六つの操作だけが出入口として開いてゐる
  （番号を受け取る・進捗を読む・進捗を書く・解答を記録する・組む・成立を記録する）。
  どの出入口も、最初に「その参加者 ID が名簿に居るか」を確かめる
...
1. `.../v2/index.html?team=team1&debug=1` を開き、「調査を開始する」を押す。
   何も打たずに番号が渡り、記録の画面へ進めば成功。
2. もう一度同じ URL を開いて押すと、**別の番号**が渡る。
   同じ番号が二度渡るなら手順 2 の `schema.sql` が古い。貼り直すこと。
3. 「この組の札が尽きた」と出たら、手順 3 の人数を使ひ切つてゐる。
   `tools/ids.html` で増やして SQL を追加投入する。

次に、**端末を替へても進捗が続くか**を試す。
